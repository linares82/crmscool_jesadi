  ...
  <body>
    <div class="datagrid">
        ...
        <table class="table table-condensed table-striped">
            <thead>
                <th>Plantel</th><th>Especialidad</th><th>Grupo</th><th>Alumno</th><th>Fecha</th><th>Estatus</th><th>Tel. Cel.</th><th>Descripción</th><th>Inscripciones Activas</th>
            </thead>
            <tbody>
                <?php $cont=0; ?>
                @foreach($registros as $registro)
                <tr>
                    <td>{{ $registro->razon }}</td><td>{{ $registro->especialidad }}</td><td>{{ $registro->grupo }}</td><td>{{$registro->cliente."-".$registro->nombre." ".$registro->ape_paterno." ".$registro->ape_materno}}</td>
